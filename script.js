/*
 * CRISTALHILLS — SECURE VERSION
 * Minecraft server website + Supabase
 *
 * ИЗМЕНЕНИЯ:
 * 1. Rate limiting (5 попыток входа в минуту)
...
// ✅ Rate limiting: храним timestamp попыток входа
let loginAttempts = [];
const MAX_LOGIN_ATTEMPTS = 5;
const LOGIN_WINDOW_MS = 60 * 1000; // 1 минута
...
function checkRateLimit() {
    ...
    if (loginAttempts.length >= MAX_LOGIN_ATTEMPTS) {
        ...
        const waitTime = Math.ceil((LOGIN_WINDOW_MS - (now - oldestAttempt)) / 1000);
        return {
            allowed: false,
            message: `⚠️ Слишком много попыток. Подождите ${waitTime} сек.`
        };
    }
...
// ✅ ИЗМЕНИТЬ: handleLogin с rate limiting
async function handleLogin(event) {
    ...
    if (!rateCheck.allowed) {
        showError('login-error', rateCheck.message);
        return;
    }
    ...
    try {
        ...
        // ✅ ПРОВЕРКА СУЩЕСТВОВАНИЯ через безопасную функцию
        const existsResult = await client.rpc('username_exists', {
            p_username: username
        });
...
// ✅ ИЗМЕНИТЬ: handleRegister с усиленной проверкой пароля
async function handleRegister(event) {
    ...
    if (!passwordCheck.valid) {
        showError('register-error', passwordCheck.message);
        return;
    }
...
// ✅ ИЗМЕНИТЬ: changePassword с проверкой сложности
...
// Функции уже объявлены выше, просто добавляем их в window
window.navigateTo = navigateTo;
window.switchAdminTab = switchAdminTab;
window.switchToServer = switchToServer;
window.deleteServer = deleteServer;
window.loadAdminSettings = loadAdminSettings;
window.addNewIpField = addNewIpField;
window.addNewBuildField = addNewBuildField;
...
window.showNewQuestionModal = showNewQuestionModal;
window.closeModal = closeModal;
window.openQuestionView = openQuestionView;
window.closeViewModal = closeViewModal;
window.submitAnswer = submitAnswer;
window.completeQuestion = completeQuestion;
window.playerCompleteQuestion = playerCompleteQuestion;
